...
- هوية بصرية كاملة: شعار من أيقونة (ثلاثة أسطر متن وحاشية) وكلمة «متن» مرسومة مساراتٍ من خط ثمانية دون شحن ملف الخط، ومادة زجاجية شبيهة بـLiquid Glass على طبقة التحكم وحدها.
- صفحة هبوط عربية على جذر GitHub Pages، والقارئ الحي ينتقل إلى `/try/`. تُبنى من `site/landing.html` وتستورد كتلة ألوان القارئ نفسها فلا تنحرف عنها.
- `scripts/screenshots.mjs` يولّد لقطات README وصفحة الهبوط من التطبيق الحي، ونتيجته متطابقة بايتًا بين التشغيلات.
- `scripts/wordmark.py` يوثّق كيفية إخراج مسارات الشعار: `hb-shape` للوصل الحرفي ثم `fontTools` للمخططات.
...
- السمات الأربع انتقلت إلى عائلة لون واحدة: ورق من الألباستر وأكسنت زيتوني في الفاتحة، وأرض زيتونية وأكسنت نعناعي في الداكنة. كل زوج نص/خلفية يتجاوز WCAG AA، مقيسًا لا مقدّرًا — وقيمتان كانتا ترسبان قبل ذلك (`--muted` عند 3.29:1 في الفاتح و3.08:1 في سيبيا).
- الواجهة صار لها مكدّس خطوط خاص (`--ui`) فتوقف خط القراءة الذي يختاره القارئ عن التسرّب إلى القوائم والحوارات.
...
- `var(--ui)` كان مستخدمًا في أربعة عشر موضعًا بلا تعريف، فكان كل `font:` مختصر يحتويه باطلًا: `.primary` كان يُرسم 18px/400 بدل 14px/600 المقصود، و`.preset-copy` 18px بدل 11px.
- الترويسة صارت `flex:none`. كعنصر flex في عمود مع `flex-shrink:1` كانت تنكمش إلى 35px عند فيض المستند، فتفسد كل ثابت مبني على 52px: `.panel{top:52px}` و`scroll-margin-top:66px` و`.findbar{top:64px}` وثلاث قواعد في استعلام 820px.
- أربع قواعد كانت تكتب `color:#fff` فوق `var(--accent)` — نسبة 1.67:1 مع الأكسنت النعناعي. صار لها `--on-accent`.
- عرض الديمو على GitHub Pages: `new Worker("/render-worker.js")` لم يكن يُحوَّل ولا يُنسخ، فالمستندات الأكبر من 2MB كانت تصطدم بـ404.
