...
### Added
- لصق ماركداون من الحافظة (`⌘V`) وعرضه فورا، مع تراجع يعيد المستند الذي كان مفتوحا.
  الصيغة تصل من الحافظة بقدر ما تصل كملف، وكان هذا المسار الوحيد الناقص.
- إشارة إلى موضع التغيير عند تعديل الملف من الخارج: الموضع يبقى كما هو، ورسالة تسمّي
  القسم الذي تغيّر وتعرض الانتقال إليه — دون أن تقفز بك إليه.
- ترشيح العناوين داخل الفهرس للمستندات الطويلة (أكثر من ثمانية عناوين)، بتطبيع عربي
  مشترك مع بحث المكتبة فلا تعطّله الهمزات ولا التشكيل.
- قائمة اختصارات لوحة المفاتيح، تُفتح بـ`؟` أو من لوحة الإعدادات. الاختصارات نفسها
  كانت تعمل من قبل لكنها لم تكن مذكورة في الواجهة إطلاقًا.
- [ARCHITECTURE.md](./ARCHITECTURE.md)، وملف مساهمة، وقواعد سلوك، ونماذج للمسائل
  وطلبات الدمج.

### Fixed
- التحديث الفوري يحافظ على الأقسام المطوية وعلى موضع القراءة بالضبط. كان كل حفظ من
  المحرّر يفتح كل الأقسام ويزيح الموضع (قِيس ‏800px على ملف 97KB) — وهو أسوأ ما يظهر
  عند العمل مع أدوات تكتب في الملف باستمرار.
- المعادلات المستقلة تتوسّط عمود القراءة بدل التصاقها بحافته.
- تصدير EPUB يُنتج XHTML سليمًا: كانت `<hr>` و`<br>` و`<img>` ومربّعات المهام تخرج
  غير مغلقة، فيرفض القارئ المتوافق الملف من أول عنصر.
- الفقرة المكتوبة كاملة بحروف لاتينية داخل مستند عربي تُقرأ من اليسار؛ والعناوين
  التي تبدأ بكلمة إنجليزية تبقى من اليمين كما كانت.
- المعرّفات العربية داخل كتل الكود تُعزل بـ`<bdi>`، فلا ينقلب `const مرحبا = "أهلا";`.
- مفاتيح البيانات الوصفية وعنوان بطاقتها تقرأ باتجاه محتواها، وبطاقة البيانات لم تعد
  تفيض عن عرضها.
- عنوان قسم الحواشي مترجم، ولم يعد يظهر كفصل وهمي في الفهرس (هو للقارئات الصوتية فقط).
- عدّادات أقسام لوحة التفاصيل لم تعد تلتصق بعناوينها.
- أهداف النقر الصغيرة (سهم الطي، ورابط العنوان، وعلامة سطر المصدر) صارت 24px على
  الأقل، وفق WCAG 2.5.8.

### Security
- كل مسارات `/api` ترفض الطلبات القادمة من أصل آخر أو من منفذ آخر على 127.0.0.1.
  كان `/api/list` وحده محميًّا، بينما `/api/file-info` يشغّل `git` لكل طلب.
- مقارنة رمز الجلسة صارت عبر `timingSafeEqual`.

### Changed
- أعيد بناء الواجهة وفق بنية Liquid Glass الحديثة: مكتبة جانبية مستقلة، مساحة قراءة
  معتمة، ولوحة تفاصيل منفصلة للفهرس والملاحظات ومعلومات المستند.
- صار شريط الأدوات ثلاث مجموعات وظيفية عائمة، مع بحث داخل المكتبة، ومواد منكسرة
  للأدوات المؤقتة، وخلفيات ممتدة خلف الألواح، ودعم تقليل الشفافية والحركة.
- مخططات Mermaid تعيد التصيير عند تبديل السمة بدل الاحتفاظ بألوان السمة السابقة.

## [1.3.0] — 2026-07-30

أول إصدار يُنشر على npm باسم `@ajarallah/matn`؛ الاسم العام `matn` تحجزه حزمة أخرى.
والأمر التنفيذي يبقى `matn` كما هو.

### Added
- فتح وفهرسة صيغ Markdown الشائعة: `.mkdn` و`.mdwn` و`.mdtxt` و`.mdtext`
  و`.rmd` و`.qmd`، إلى جانب الصيغ المدعومة سابقًا، دون إدخال ملفات `.txt` العامة في الفهرس.
...
### Fixed
- **الجوال يعمل مجددًا.** كان الشريط الجانبي يبقى بعرضه الثابت على الشاشات الضيقة فيهبط عمود
  القراءة إلى ٥٥ بكسل وتتقطّع الكلمات؛ صار الشريط درجًا كما صُمّم، لأن ظهوره لم يعد يُكتب
  كنمط `style` سطري يتغلّب على قواعد التجاوب.
- **أسماء الملفات لا تنقلب.** كان `010-market-research.md` يُعرض `market-research.md-010` في
  شاشة البداية وزر المتابعة والمفضلة وترويسة الملف؛ صارت هذه المواضع تتبع أول محرف قويّ.
- **تعليقات HTML لم تعد تظهر للقارئ.** `<!-- ... -->` كانت تُهرَّب فتُطبع نصًا؛ صارت تُسقَط من
  النثر وتبقى كما هي داخل الكود المسيَّج والسطري.
- **لوحة الإعدادات تفتح تحت زرّها** بدل الطرف المقابل من الشاشة، ولا تغطّي الشريط الجانبي.
- **خريطة المستند لا تركب فوق شجرة الملفات.**
- **يمكن حذف التمييز والملاحظة**، ويُختار اللون عند التمييز مباشرة بدل أربع خطوات، ولكل عملية تراجع.
- **`#` و`¶` لا تدخل في نسخ العناوين.**
- **«سلامة المستند» لا تُنذر كذبًا** بروابط تشير خارج المجلد المفتوح؛ تُعرض دون أن تُعدّ عطلًا.
- أزرار الشريط العلوي صار لها اسم معلن لقارئ الشاشة، وروابط الملفات تعلن مسارًا نسبيًا لا مطلقًا.
...
### Distribution
- **التثبيت صار ١٤ ثانية بدل دقيقتين.** كان اختصار GitHub يستنسخ المستودع بتاريخه
  (~٣٥ ميغابايت) ثم يقلّمه؛ و`.gitattributes` يستبعد الآن ما لا يعمل وقت التشغيل
  فهبط أرشيف المصدر من ٧.٦ إلى ١.٩ ميغابايت.
- **النقر المزدوج على macOS لم يعد يحتاج Homebrew.** يجرّب المثبِّت `duti`، ثم نفس نداء
  LaunchServices بسطر Swift، ثم يرشد إلى Get Info إن تعذّرا.
- `install-macos.sh --uninstall` يعيد `.md` إلى تطبيقك السابق وينقل التطبيق إلى سلة النظام.
- توثيق التثبيت صار يعطي مسار التثبيت العام الصحيح، ويوثّق الإزالة لأول مرة.
